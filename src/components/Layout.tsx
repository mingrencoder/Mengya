...
import { Compass, Book, User, Settings, Command } from 'lucide-react';
...
export function Layout() {
  return (
    <div className="relative h-screen w-full flex overflow-hidden flex-col lg:flex-row">
      ...
      <aside className="hidden lg:flex flex-col w-64 h-full fixed left-0 top-0 border-r border-white/10 glass z-50 py-6 px-4">
        <div className="flex items-center gap-3 px-4 mb-10">
          <div className="w-8 h-8 rounded-lg bg-indigo-500 flex items-center justify-center shadow-lg shadow-indigo-500/20">
            <span className="font-bold text-white text-xs">萌</span>
          </div>
          <span className="font-semibold text-white tracking-tight">萌芽</span>
        </div>

        <nav className="flex-1 space-y-1 overflow-y-auto">
          <div className="text-[10px] uppercase tracking-widest text-slate-500 font-bold px-4 mb-2">概览</div>
          {NAV_ITEMS.map((item) => (
            <NavLink
              key={item.path}
              to={item.path}
        ...
        <div className="mt-auto pt-4 space-y-1">
          <div className="text-[10px] uppercase tracking-widest text-slate-500 font-bold px-4 mb-2">账户</div>
          <NavLink
            to="/admin"
            className={({ isActive }) => isActive ? "sidebar-item-active" : "sidebar-item"}
          >
      ...
      {/* Main Content Area */}
      <main className="flex-1 lg:ml-64 relative z-10 w-full mb-20 lg:mb-0 h-full flex flex-col pt-[max(env(safe-area-inset-top),_0px)]">
